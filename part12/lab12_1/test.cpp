    ...
    public:
        ...
        void runline(string cmdline)//运行一行参数
        {
            ...
            case 'c':
                ...
                else if(day>date.getmaxday()||day<=0)
                {
            ...
            case 'q':
                ...
                if(m1<=0||m1>12||d1<=0||d1>Date::getmaxday(y1,m1))
                    throw runtime_error("invalid date");
                if(m2<=0||m2>12||d2<=0||d2>Date::getmaxday(y2,m2))
                    throw runtime_error("invalid date");
...
inline void runcmdline(Controller &cmd,string &cmdline)
{
    try
    {
        cmd.runline(cmdline);
    ...
    {
        cout << "Error(#" << e.getacc()->getid()<< "):" << e.what() << endl;
    }
    catch(exception &e)
    {
...
int main()
{
    ...
    if(c=='y')
    {
        ...
        if(infile)
        {
            ...
            {
                runcmdline(cmd, cmdline);
                //cmd.runline(Controller cmd,cmdline);
            }
        }
        infile.close();
    }
